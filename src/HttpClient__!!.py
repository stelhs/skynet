...
class HttpClient():
    def __init__(s, name, host, port, clientHost):
        s.clientHost = clientHost
        s.host = host
        s.port = port
        s.name = name
        s.lock = threading.Lock()
        s.log = Syslog('http_client_%s' % s.name)

        s.conn = None
        s.reqQueue = []
        s.task = Task('http_client_%s' % s.name, s.close)
    ...
    def doTask(s):
        while 1:
            ...
            for item in queue:
                ...
                with s.lock:
                    s.reqQueue.remove(item)


    def notify(s, type, data):
        with s.lock:
            s.reqQueue.append((type, data))
